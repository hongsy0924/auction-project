...
export interface ParsedQuery {
    council: string | null;
    keywords: string[];          // Multiple search terms, broadest first
    intent: string;
    analysisFocus?: string[];    // What aspects to look for (e.g. "예산 배정", "사업 진척")
}
...
export async function summarizeMinutes(
    ...
    const context = minutes
        .map(
            (m) => `
[Date: ${m.date}]
[Meeting: ${m.meeting}]${m.speaker ? `\n[Speaker: ${m.speaker}]` : ""}${m.agendaContext ? `\n[Agenda: ${m.agendaContext}]` : ""}
[Excerpt]:
${m.content}
---
`
