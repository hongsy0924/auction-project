...
export interface ScoringInput {
    ...
    gosiStage?: number;
    /** min_to_official_ratio (최저가/공시지가총액) */
    minToOfficialRatio?: number | null;
    /** 유찰 횟수 */
    yuchalCount?: number;
}
...
/** Calculate price attractiveness score (0-1.0). */
function scorePriceAttractiveness(input: ScoringInput): number {
    const ratio = input.minToOfficialRatio;
    if (ratio == null || ratio <= 0) return 0;

    for (const tier of SCORING_CONFIG.price_attractiveness) {
...
export function calculateScore(input: ScoringInput): ScoreBreakdown {
    ...
    const pa = scorePriceAttractiveness(input);
    ...
    const total =
        ...
        pa * weights.price_attractiveness +
        tm * weights.timing;
    ...
    return {
        ...
        components: {
            ...
            price_attractiveness: { raw: pa, weighted: pa * weights.price_attractiveness },
